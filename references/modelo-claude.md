...
Toda vez que uma entidade, relacionamento ou fluxo crítico muda, `docs/UML.md` é atualizado no **mesmo commit** que muda o código — nunca depois. Uma mudança de schema/domínio commitada sem o UML correspondente é tratada como bug, não como pendência.
...
| `docs/UML.md`                | Diagramas de classes/entidades e sequência (Mermaid) — **bloqueia código novo/commit se ausente/desatualizado** | — |
...
- [ ] Toda entidade/relacionamento/fluxo crítico alterado nesta entrega está refletido no `docs/UML.md`, no mesmo commit?
